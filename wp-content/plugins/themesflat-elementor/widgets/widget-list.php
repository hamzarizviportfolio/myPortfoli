...
class TFList_Widget extends \Elementor\Widget_Base {
	...
	private function register_section_content() {
		...
		$this->add_control(
			...
			[
				...
				'options'        => [
					...
					'list-type-rectangle' => esc_html__( 'List Rectangle', 'themesflat-elementor' ),
				],
				'style_transfer' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'        => 'circle',
				'options'        => [
					'custom' => esc_html__( 'List Custom', 'themesflat-elementor' ),
	...
	private function register_section_list_style() {
		...
		$this->add_responsive_control( 'list_icon_space_between',
			[
				...
				'selectors'  => [
					...
					'{{WRAPPER}} .list-unstyled .list-icon-item:not(:last-child)' => 'margin-bottom: calc({{SIZE}}{{UNIT}}/2);',
				],
			]
		);
		...
		$this->add_control(
			'list_animation_hover',
			[
				'label'     => esc_html__( 'Animation When Hover', 'themesflat-elementor' ),
		...
		$this->add_group_control(
			...
			[
				...
				'condition'        => [
					'list_icon_type!' => 'custom',
				],
			]
		);
		$this->add_responsive_control(
			'list_icon_custom_width',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .tf-list-icon-custom .list-icon-item::before' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			'list_icon_custom_height',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .tf-list-icon-custom .list-icon-item::before' => 'height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors'          => [
					'{{WRAPPER}} .tf-list-icon-custom .list-icon-item::before' => 'bottom: {{TOP}}{{UNIT}}; left:{{RIGHT}}{{UNIT}};',
				],
				'condition' => [
					'list_icon_type' => 'custom',
				],
	...
	protected function render( $instance = [] ) {
		...
		$list_icon_classes = array(
			'tf-list-icon',
			'tf-' . $settings['list_icon_view']
		);

		if($settings['list_animation_hover'] ==="yes"){
			$list_icon_classes[] = 'has-hover-animation';
        }
