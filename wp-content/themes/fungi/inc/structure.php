...
if ( ! function_exists( 'themesflat_body_classes' ) ) {
	...
	function themesflat_body_classes( $classes ) {
		...
		if ( $header_sticky == 1 ) {
			$header_sticky_mode = themesflat_get_option( 'header_sticky_mode' );

			$classes[] = 'header_sticky';
			$classes[] = $header_sticky_mode;
		}
		 ...
		 * One Page
		 */
		$one_page_enable = themesflat_get_opt_elementor( 'one_page_enable' );
		if ( $one_page_enable == "yes" ) {
			$classes[] = 'is_one_page';
		}
