...
function themesflat_shortcode_default_id() {
	return array(
		...
		'group'      => esc_html__( 'Design Options', 'fungi' ),
...
function themesflat_predefined_header_styles() {
	...
	if ( empty( $styles ) ) {
		$styles = apply_filters( 'themesflat/header_styles', array(
			'header-v1' => esc_html__( 'Classic', 'fungi' ),
			'header-v2' => esc_html__( 'Header style 02', 'fungi' ),
			'header-v4' => esc_html__( 'Modern', 'fungi' ),
		) );
	}
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . esc_html__( 'Create a Menu', 'fungi' ) . '</a></li></ul>';
}
...
function themesflat_blog_layout() {
	switch ( get_post_type() ) {
		...
		case 'services':
			...
			if ( is_single() ) {
				$layout = themesflat_get_opt( 'services_single_layout' );
			}
			if ( isset( $_GET['services_layout'] ) ) {
				$layout = $_GET['services_layout'];
			}
			break;
		case 'portfolio':
			$layout = themesflat_get_opt( 'portfolio_layout' );
			if ( is_single() ) {
				$layout = themesflat_get_opt( 'portfolio_single_layout' );
			}
			break;
		default:
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	...
	function themesflat_wp_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'fungi' ), max( $paged, $page ) );
		}
...
function themesflat_set_opt( $key, $value ) {
                ...
                <ul class="unstyled">
                    <li class="current">
                        <a href="?lang=en" class="lang_sel_sel">
                            <span class="languages-before-icon fa fa-globe"></span><?php echo esc_html__( "English", 'fungi' ); ?>
                            <i class="far fa-chevron-down" aria-hidden="true"></i>
                        </a>
                        <ul class="unstyled-child">
                            <li class="icl-en">
                                <a href="?lang=en" class="lang_sel_sel">
									<?php echo esc_html__( "English", 'fungi' ); ?>
                                </a>
                            </li>
                            <li class="icl-fr fr">
                                <a href="?lang=fr" class="lang_sel_other">
									<?php echo esc_html__( "French", 'fungi' ); ?>
                                </a>
                            </li>
                            <li class="icl-ge ge">
                                <a href="?lang=it" class="lang_sel_other">
									<?php echo esc_html__( "German", 'fungi' ); ?>
                                </a>
                            </li>
                        </ul>
                    </li>
...
function themesflat_custom_search_form( $form ) {
   ...
   <div class="input-group">
    <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", 'fungi' ) . '"/>
...
function themesflat_social_single() {
	if ( themesflat_get_opt( 'show_social_share' ) == 1 ):
        ...
        <div class="social-share-article">
            <label class="social-share-label"><?php esc_html_e('Share:','fungi')?></label>
...
function themesflat_get_page_titles() {
	...
	if ( ! is_archive() ) {
		if ( is_home() ) {
			...
			$title = esc_html__( '404', 'fungi' );
		} elseif ( is_search() ) {
			$title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'fungi' ), get_search_query() );
		} else {
			$title = get_post_meta( get_the_ID(), 'custom_title', true );
			if ( ! $title ) {
				$title = get_the_title();
		...
		if ( is_author() ) {
			the_post();
			$title = sprintf( esc_html__( 'Author: %s', 'fungi' ), get_the_author() );
			rewind_posts();
		} elseif ( is_day() ) {
			$title = sprintf( esc_html__( 'Daily: %s', 'fungi' ), get_the_date() );
		} elseif ( is_month() ) {
			$title = sprintf( esc_html__( 'Monthly: %s', 'fungi' ), get_the_date( 'F Y' ) );
		} elseif ( is_year() ) {
			$title = sprintf( esc_html__( 'Yearly: %s', 'fungi' ), get_the_date( 'Y' ) );
		} elseif ( is_search() ) {
			$title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'fungi' ), get_search_query() );
...
function themesflat_svg( $icon ) {
	$svg = array(
		'cart'     => '<i class="icon-fungiadd-to-basket-1"></i>',
